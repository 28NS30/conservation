...
export default async function SpeciesPage({
  ...
  return (
    <main className="mx-auto min-h-[100dvh] w-full max-w-2xl px-4 pb-16 pt-5">
      <Link href="/species" className="text-xs text-slate-400 hover:text-slate-200">
        {nav("backToSpecies")}
      </Link>

      <header className="mt-3">
        <h1 className="text-xl font-semibold text-slate-50">{headline}</h1>
        {secondary && (
          <p className="mt-0.5 text-sm text-slate-400">
            <span className="italic">{secondary}</span>
            {s.nameAuthor && <span className="ml-1.5 not-italic text-slate-500">{s.nameAuthor}</span>}
          </p>
        )}
      ...
      {lineage.length > 0 && (
        <nav aria-label={t("taxonomy")} className="mt-4 text-[11px] text-slate-500">
          {lineage.join(" › ")}
          {habitats.length > 0 && <span className="ml-2 text-slate-600">· {habitats.join(" / ")}</span>}
        </nav>
      )}
      ...
      <section className="mt-6">
        ...
        {withheld ? (
          ...
          <div className="mt-2 rounded-lg border border-white/10 bg-slate-900/50 px-3 py-4 text-center">
            <p className="text-sm text-slate-400">{t("noRecords")}</p>
            <Link
              href="/report"
              className="mt-3 inline-block rounded-full bg-emerald-500 px-3.5 py-1.5 text-xs font-semibold text-slate-950 hover:bg-emerald-400"
            >
              + {t("beFirst")}
            </Link>
          </div>
          ...
          <>
            ...
            {records.length > 0 && (
              <ul className="mt-3 space-y-1 text-xs">
                {records.map((r) => (
                  <li key={r.id} className="flex items-center justify-between gap-3 rounded border border-white/10 px-2.5 py-1.5">
      ...
      {!withheld && s.reportCount > 0 && (
        <Link
          href="/report"
          className="mt-6 inline-block rounded-full border border-white/15 px-3.5 py-1.5 text-xs text-slate-300 hover:bg-slate-800"
        >
          + {t("seenOne")}
        </Link>
      )}
